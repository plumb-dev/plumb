...
export class RegistryLoader {
  ...
  // ── Load the snapshot bundled inside the published package ───────────────
  //
  // When @plumb/scanner is installed from npm there's no monorepo registry
  // directory, so consumers fall back to registry.bundled.json shipped at the
  // package root (generated by `npm run bundle` at pack time). Returns false if
  // the snapshot isn't present (e.g. running from source before bundling).

  loadBundledDefault(): boolean {
    const bundlePath = path.join(__dirname, '..', '..', 'registry.bundled.json');
    if (!fs.existsSync(bundlePath)) return false;
    this.loadBundled(bundlePath);
    return true;
  }
